...
import com.asakusafw.spark.runtime.serializer.DataModelSerializer
...
class KryoSerializerClassBuilder(flowId: String, dataModelType: Type)
    extends ClassBuilder(
      ...
      classOf[DataModelSerializer[_]].asType) {
  ...
  override def defMethods(methodDef: MethodDef): Unit = {
    ...
    methodDef.newMethod("newInstance", classOf[DataModel[_]].asType, Seq.empty) { mb =>
      import mb._
...
object KryoSerializerClassBuilder {
  ...
  def signature(dataModelType: Type): String = {
    new ClassSignatureBuilder()
      .newSuperclass {
        _.newClassType(classOf[DataModelSerializer[_]].asType) {
