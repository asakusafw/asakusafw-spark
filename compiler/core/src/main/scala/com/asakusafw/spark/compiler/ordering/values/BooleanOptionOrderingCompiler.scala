...
class BooleanOptionOrderingCompiler extends OrderingCompiler {
  def of: Type = classOf[value.BooleanOption].asType
  def compare(x: Var, y: Var)(implicit mb: MethodBuilder): Stack = {
    import mb._
    assert(x.`type` == of)
    assert(y.`type` == of)
    getStatic(orderings.BooleanOptionOrdering.getClass.asType, "MODULE$", orderings.BooleanOptionOrdering.getClass.asType)
