...
class StringOptionOrderingCompiler extends OrderingCompiler {
  def of: Type = classOf[value.StringOption].asType
  def compare(x: Var, y: Var)(implicit mb: MethodBuilder): Stack = {
    import mb._
    assert(x.`type` == of)
    assert(y.`type` == of)
    getStatic(orderings.StringOptionOrdering.getClass.asType, "MODULE$", orderings.StringOptionOrdering.getClass.asType)
