...
class DateTimeOptionOrderingCompiler extends OrderingCompiler {
  def of: Type = classOf[value.DateTimeOption].asType
  def compare(x: Var, y: Var)(implicit mb: MethodBuilder): Stack = {
    import mb._
    assert(x.`type` == of)
    assert(y.`type` == of)
    getStatic(orderings.DateTimeOptionOrdering.getClass.asType, "MODULE$", orderings.DateTimeOptionOrdering.getClass.asType)
