...
class InputOutputDriverClassBuilderSpec extends FlatSpec with SparkWithClassServerSugar {
  ...
  it should "build input and output driver class" in {
    ...
    val outputSubPlan = PlanBuilder.from(Seq(outputOperator))
      .add(
        Seq(outputMarker),
    ...
    // prepare for input
    val sn = outputSubPlan.getElements.head.getOperators.toSet[Operator]
      .find(_.getOriginalSerialNumber == outputOperator.getOriginalSerialNumber).get.getSerialNumber
    ...
    val inputSubPlan = PlanBuilder.from(Seq(inputOperator))
      .add(
        Seq(beginMarker),
