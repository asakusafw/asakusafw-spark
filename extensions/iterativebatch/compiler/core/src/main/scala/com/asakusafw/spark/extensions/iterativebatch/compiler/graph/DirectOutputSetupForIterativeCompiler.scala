...
object DirectOutputSetupForIterativeCompiler {
  ...
  def compile(
    ...
    val setup = context.addClass(
      new DirectOutputSetupClassBuilder(
        outputs.map { output =>
          val model = DirectFileIoModels.resolve(output.getInfo)
          (output.getName, model.getBasePath, model.getDeletePatterns.toSeq)
        }) with CacheAlways)

    val builder = new DirectOutputSetupForIterativeClassBuilder(setup) with CacheOnce
